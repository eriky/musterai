...
import { User, Role, Invitation, Agent, AuthMe, AuditRecord } from '../types.js';
import { api, ApiError } from '../api.js';
import { ALL_PERMISSIONS, Permission } from '../../shared/permissions.js';
import {
  Users, ShieldCheck, Mail, Trash2, Plus, X, Copy, Check,
  Bot, ChevronDown, ChevronRight, AlertCircle, ScrollText, RefreshCw,
} from 'lucide-react';
...
const AuditLogPanel: React.FC<{
  ...
  agents: Agent[];
  onError: (msg: string) => void;
}> = ({ workspaceId, users, agents, onError }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs muster-text-muted">
          Privileged actions only — role changes, membership, tokens, invitations, document approvals, project deletion.
          Never writable directly; every row is a side effect of the action it records.
        </p>
